...
1. ~~**GDN 하이브리드는 구조적으로 불가**~~ **🔴 철회 (§46).** "트리 GDN 커널이
   스텝 단가를 +44~49% 올린다" 는 측정이 아니라 추론이었다. 직접 재니 우리
   동작점(T=8)에서 트리 커널은 스톡 커널보다 **1.2 µs/계층** 비쌀 뿐이다
   (48계층 = 0.06 ms, 격차의 0.3%). 실제로는 **사슬 모양·마스크 없음에서도 나는
   18 ms 의 미귀속 GPU 비용**이 격차의 89% 이고, 아직 못 찾았다.
...
## 살아남은 것 (재사용 가능)
  ...
  - **RoPE 기준 버그 수정** (`d6d1f39f8e`) — `num_computed_tokens_np` 는 낙관적
...
배치(§41) · 모델·드래프터(§42) · 문맥 길이(§37) · TP(§43) · **온도(§45)** 전부
쟀다. 온도는 운영 모델에서 0.660x → 0.675x 로 1.5%p 만 바꾼다 — 온도가 도울 수
있는 건 '분기가 헛돎' 뿐이고 GDN 커널의 단가는 온도와 무관하기 때문이다.

**수용의 상한도 쟀다 (§50, 2026-09-02).** 오라클 트리(척추 7 + 형제)로 모든 부분
모양을 한 번에 재니, 트리 쪽에 남은 수용은 **+3.5~4%(형제 둘)** 뿐이고 형제 뒤는
주변분포가 척추 조건부라 22% 만 이어진다. 기계값을 전부 없애고 GDN 커널을 T≤16
으로 넓혀도 **0.94x**. 운영 샘플링 조건(T=1.0, top_k 20)에서는 수용에서도 진다.
**DFlash2 를 넘는 길은 드래프터 쪽뿐이다.**
...
6. **최저값으로 팔을 비교하지 말 것** (§44). 분산이 다르면 최소값 비교는 편향
   ...
   중앙값 + 부호검정을 쓸 것.
7. **27B 잡음은 운영 GPU1 온도에 좌우된다** (§49-7). 92°C/100% 면 sd 0.5% 라 0.3%
   효과는 40쌍이 필요하다. 세밀 측정 전 `nvidia-smi` 를 볼 것.
8. **'효과 없음' 과 '검정력 부족' 을 가를 것** (§49-7). 구간이 줄었는데 끝에서
   끝까지 안 보이면 잡음 sd 와 효과 크기를 같이 적는다.
9. **한 trace 로 계산한 부분 모양 반사실은 위치 편향이 있다** (§50-4). 트리가
   멈춘 위치는 체계적으로 더 어렵다 — 반사실 +15% 가 끝에서 끝까지 +3.4% 였다.
   조건부 확률에만 쓰고 토큰/스텝은 스텝 수로 잴 것.
...
순서에 의존하지 않게 되므로 함께 커밋했다 (`d6d1f39f8e`).
...
검사 두 가지.
  ...
  - 기본값이 무연산인가 — δ=0 에서 변경 전 코드(`d6d1f39f8e`)와 무작위 300
...
절감 0.39 ms = 0.59 × 0.69 ms — 산수가 정확히 맞는다. 커밋 `166ad3c298`.
...
# 45. 온도 > 0 — 구현하고 재봤다, 판정은 그대로 (2026-09-02)

## 45-1. 그냥 온도만 올리면 비교가 성립하지 않는다

DDTree 의 `accept()` 는 `logits.argmax` 뿐이었다. 온도만 올리면 사슬은 샘플링하고
DDTree 는 그리디를 내놔 **분포 자체가 달라진다.**

트리에 맞는 분포 보존 수용은 구현할 수 있다. 노드 i 의 logits 은 조상 문맥으로
계산되므로(트리 마스크의 정의), **노드마다 타깃 분포에서 뽑고 그 토큰으로 트리를
내려가면** 방출 열의 매 토큰이 그 위치의 p 에서 뽑힌 것이 된다. 온도 0 의 argmax
가 그 특수경우다.

vLLM 자신의 `gumbel_sample` 을 그대로 쓰되 **노이즈 위치를 노드의 깊이 위치**
(= 그 노드가 차지할 출력 위치, 깊이 RoPE 가 이미 계산해 둔 값)로 준다. 같은 깊이
형제는 같은 노이즈를 쓰지만 **수용 경로에는 깊이마다 노드가 하나뿐**이라 무해하다.

🔴 **이건 기각 샘플링이 아니다.** vLLM 사슬은 `min(1, p/q)` 기각 샘플링으로
수용률이 `Σ min(p,q)` 인데, 이 방식은 `Σ q·p` 라 **구조적으로 낮다.** 즉 아래
비교는 DDTree 에게 불리한 조건이다.

## 45-2. 검증

  - **온도 0 에서 기존과 바이트 동일** (6/6, 카운터도 전부 일치 — 688스텝, 수용
    545). `gumbel_noised_argmax` 가 온도 0 에서 순수 argmax 라 그렇다.
  - **샘플러가 실제로 동작한다.** DDTree T=0.7 출력은 자기 T=0 출력과 위치별
    **5.7%** 만 겹친다(그리디면 ~100%). 엔트로피 8.251 bit 로 base(8.231),
    사슬(8.176)과 같은 대역이고 고유 토큰 수도 비슷하다(544 / 551 / 529).

🔴 **분포 정확성의 증명은 아니다.** 시드를 고정해 base 와 대조하려 했으나
**사슬도 base 와 0/6** 이었다 — 기각 샘플링은 난수를 다르게 소비해 노이즈 열이
갈린다. 그래서 이 검사로는 못 가린다. 근거는 위 해석적 논증과, DDTree 가
base 와 2/6 으로 사슬(0/6)보다 더 일치한다는 사실(같은 노이즈 열을 쓰므로
예상되는 방향)뿐이다. 배포하려면 경험적 분포 비교가 따로 필요하다.

## 45-3. 결과 — 8B 순수 어텐션 + EAGLE3, 배치 1

| 온도 | 사슬 예산3 | DDTree | 사슬 대비 | 수용 (사슬 → 트리) |
|---|---|---|---|---|
| 0 | 23.29s | 23.28s | 1.00x | 1.498 → 1.761 |
| 0.7 | 23.30s | 23.28s | **1.00x** | 1.492 → 1.732 |
| 1.0 | 23.59s | 24.44s | **0.965x** | 1.475 → 1.648 |

온도가 올라갈수록 **DDTree 의 수용이 더 빨리 떨어진다** (1.761 → 1.732 → 1.648
대 사슬 1.498 → 1.492 → 1.475). §45-1 의 `Σq·p` 대 `Σmin(p,q)` 격차가 온도와 함께
벌어지는 것이다.

## 45-4. 결과 — 운영 모델 (27B + DFlash2), 배치 1

| 온도 | DFlash2 사슬 예산7 | DDTree 예산7 | 사슬 대비 |
|---|---|---|---|
| 0 | 15.60s (3.200 tok/step) | 23.64s (3.097) | 0.660x |
| 0.7 | 17.17s (2.852) | 25.43s (**2.880**) | **0.675x** |

**온도 0.7 에서 처음으로 DDTree 의 수용이 사슬을 넘는다** (2.880 대 2.852).
타깃이 덜 확신하면 분기가 값을 한다는 예상대로다. 그런데 **+1.0% 수용 이득에
스텝 단가는 +45%** (63.57 대 43.69 ms) — GDN 트리 커널은 온도와 무관하다.

## 45-5. 판정

**온도는 판정을 안 바꾼다.** 운영 모델에서 0.660x → 0.675x 로 1.5%p 나아질 뿐이다.

이유가 명확하다. 온도가 도울 수 있는 건 §결론의 이유 2번(분기가 헛돎)뿐인데,
1번(GDN 커널의 구조적 단가 +45~47%)은 온도와 완전히 무관하다. 수용에서 사슬을
넘어서도 +1% 이고, 넘어야 할 적자는 +45% 다.

남은 미검증 축은 이제 없다. 배치·모델·드래프터·문맥길이·TP·온도 전부 쟀다.

# 46. 🔴 "GDN 커널이 구조적 원인" 은 틀렸다 (2026-09-02)

외부 연구 노트(katopz/katgpt-rs `.research/407_Trees_from_Marginals_GDN_Tree_Verify.md`)
가 GDN 트리 검증을 **masked triangular solve** 로 푸는 방법을 제시한다. 우리
결론의 1번 이유("GDN 하이브리드는 구조적으로 불가")를 정면으로 겨냥하므로 확인했다.

## 46-1. 우리 커널은 그들이 이기는 대역에 있다

문서의 기준선은 **per-branch** — 가지마다 재귀를 따로 돌린다. 우리 커널은 부모
인덱스로 트리를 한 번에 훑으므로 그보다 낫다. 그런데 **스케일링 모양은 같은
부류**다 (A6000, 배치1, 계층당, H=16 HV=48 DK=DV=128):

| T | 우리 트리커널 | 우리 정규화 | 문서 per-branch | 문서 masked-solve |
|---|---|---|---|---|
| 16 | 111.7 µs | 1.00x | 1.00x | 1.00x |
| 32 | 230.5 | 2.06x | 1.88x | 1.05x |
| 64 | 462.4 | 4.14x | 3.53x | 1.16x |
| 128 | 920.9 | 8.24x | 7.40x | **1.55x** |

**T 에 선형**이다. 이진트리(깊이 log T)와 사슬(깊이 T)이 거의 같은 시간인 것도
트리 구조를 병렬화에 전혀 안 쓴다는 증거다. 그들의 solve 는 거의 평평하다.

## 46-2. 그런데 우리 동작점에서 GDN 커널은 공짜다
...
|---|---|
| 우리 트리 커널 | 52.6 |
| 스톡 융합 GDN 커널 (사슬 팔이 쓰는 것) | 51.4 |
| **차이** | **1.2 → 48계층 = 0.06 ms/스텝** |

**격차 20.21 ms 의 0.3%.** masked solve 로 바꿔도 여기서 얻을 게 없다. 그들의
이득은 T≥32 에서 나오는데, DFlash2 의 학습 지평이 8 이라 우리는 못 간다.

## 46-3. 🔴 그래서 §27·§42 의 귀속이 틀렸다

"스텝 단가 +44~49% 는 GDN 트리 커널" 은 **측정이 아니라 추론**이었다. 직접 재니
반증된다. 8B 에서 쓴 격리법을 27B 에 적용해 다시 분해했다 (배치1, 예산7):

| | ms/스텝 | 토큰/스텝 |
|---|---|---|
| A DFlash2 사슬 | 43.34 | 3.200 |
| **B DDTree 사슬모양·마스크생략** | **61.34** | **3.200** |
| C + custom_mask | 63.04 | 3.200 |
| D 트리 모양 | 63.55 | 3.097 |

  - **기계값 (B−A) = 18.00 ms — 격차의 89%**. 수용이 A 와 **동일**(3.200)한데도 난다.
  - custom_mask (C−B) = 1.71
  - 트리 모양 (D−C) = 0.51

## 46-4. 그 18 ms 를 아직 못 찾았다
...
|---|---|
| GDN 재귀 커널 | +0.06 ms |
| 트리 conv (`tree_cols`) | **−0.17 ms** (오히려 빠르다) |
| CPU 작업 전부 (TIMESPLIT) | 1.60 ms |
| custom_mask | 1.71 ms (B→C 로 이미 분리됨) |
| `draft_sample_method=probabilistic` | −0.09 ms (사슬 기준선에 켜서 확인) |

합계 약 3.7 ms. **나머지 ~16.5 ms 는 미귀속 GPU 작업**이고, 사슬 모양·마스크
없음에서도 난다.

## 46-5. 판정에 대한 함의

수치 판정은 안 바뀐다 — 27B 에서 여전히 0.66x 다. 그러나 **이유가 바뀐다.**

  - 전에는 "GDN 트리 검증이 구조적으로 비싸다" 였다. **틀렸다.**
  - 실제로는 "DDTree 통합 어딘가에 18 ms 의 정체불명 GPU 비용이 있고, 그게
    격차의 89% 다." 사슬 모양에서도 나므로 **트리와 무관한 구현 오버헤드**일
    가능성이 높다.

없애면 61.34 → 43.4 ms 로 사슬과 동률이 되고, 거기서 트리 이득(온도 0.7 에서
수용 +1%)이 얹힌다. **즉 "구조적으로 불가" 가 아니라 "안 찾은 버그/오버헤드가
있다" 가 현재의 정확한 상태다.**

🔴 다음 작업은 그 16.5 ms 를 찾는 것이다. 27B 전용이고(8B 순수 어텐션의 기계값은
2.41 ms 였다) 사슬 모양에서도 나므로, GDN 계층 통합 경로(`qwen_gdn_linear_attn.py`
패치, 상태 슬롯 관리, `compact_gdn`)를 계층 프로파일러로 훑는 것이 첫 수다.

## 46-6. 문서의 나머지 부분

  - 그들의 종단 이득(+24.7% over tuned DFlash)은 **Weaver** 라는 **학습된**
    56.7M 어댑터가 만든다 — 인수분해 드래프터의 주변부 확률이 잃어버린 조건부
    의존을 복원한다. 문서 스스로 "Weaver requires gradient descent training" 이라
    적는다. 우리 결론 2번(좋은 드래프터에서 분기가 헛돎)을 정면으로 다루는 게
    그것이고, 커널이 아니다.
  - 그들의 "zero rollback" 상태 규율(검증 중 상태를 **절대 안 쓴다**, 수용 후
    단일 경로만 재생)은 우리 설계(노드마다 상태 슬롯에 쓰고 나중에 압축)와
    다르다. 16.5 ms 의 후보로 볼 만하다.

# 47. 16.5 ms 를 찾았다 — 절반은 계층마다 다시 만들던 텐서 (2026-09-02)

## 47-1. 이분법

사슬 모양(`TOPK=1`) + 마스크 생략(`CHAINMASK=1`)로 고정하면 **수용이 사슬과
정확히 같아**(3.200) 순수 기계값만 남는다. 거기서 조각을 하나씩 뺐다.

| 구성 | ms/스텝 | A 대비 |
|---|---|---|
| A DFlash2 사슬 | 43.34 | 기준 |
| B DDTree 사슬모양·마스크생략 | 61.32 | +17.99 |
| B − KV 압축 | 60.76 | +17.43 |
| B − compact_gdn | 61.23 | +17.90 |
| **B − GDN 트리경로 전부** | **49.43** | **+6.09** |

**GDN 트리 경로가 11.90 ms** 인데, 재귀 커널은 격리 측정에서 +0.06 ms 였다
(§46-2). 즉 커널이 아니라 그 주변이다.

🔴 `VLLM_DDTREE_NOGDN=1` 이 이 격리를 가능하게 한다. **사슬 모양 트리에서는
스톡 GDN 경로가 정확히 옳으므로**(폭 ≤ 8) 무손실을 깨지 않고 GDN 트리 경로만
떼어낼 수 있다.

## 47-2. 범인 — `gdn_info()` 가 계층마다 텐서를 다시 만든다

`gdn_info()` 는 **GDN 계층마다** 불린다. 27B 는 GDN 계층이 48개다
(64층 중 `linear_attention` 3 : `full_attention` 1). 그런데 결과는 계층 간
완전히 동일하다 — 같은 스텝의 같은 트리다.

그런데도 계층마다:

  - `tree_parents_tensor` → `torch.tensor(rows, device=cuda)` = **H2D 1회**
  - `tree_cols_tensor` → 파이썬 이중 루프로 numpy 를 짓고 → **H2D 1회**

**스텝당 96회의 H2D 와 48회의 파이썬 트리 순회.**

같은 파일의 `tolist_cached`/`_STEP_MEMO` 가 `state_indices` 에 대해 이 문제를
이미 풀어놨는데(주석까지 달려 있다 — "계층마다 내리면 스텝당 24회 D2H 다"),
정작 이 두 텐서는 그 처리를 안 거쳤다.

스텝당 한 번만 만들도록 고쳤다 (커밋 `ae21da9372`).

## 47-3. 결과

| | 전 | 후 | |
|---|---|---|---|
| B (사슬모양·마스크생략) | 61.32 | **51.59** | -9.73 |
| D (트리) | 63.55 | **53.78** | -9.77 |
| D 벽시계 | 23.64s | **20.00s** | |
| **DFlash2 사슬 대비** | 0.660x | **0.780x** | |

출력 6/6 바이트 동일, 수용 3.097 로 동일.
...
| 조각 | 값 |
|---|---|
| GDN 트리 경로 잔여 | 2.11 ms (전 11.90) |
| KV 압축 | 0.02 ms |
| **나머지** | **6.15 ms — 아직 못 찾았다** |

TIMESPLIT 의 CPU 작업 합계가 1.6 ms 였으므로 **약 4.5 ms 는 여전히 미귀속 GPU
작업**이다. 사슬 모양·마스크 없음에서도 나므로 트리와 무관한 통합 오버헤드다.

## 47-5. 판정에 대한 함의

**§46 의 진단이 맞았다** — "구조적으로 불가" 가 아니라 "안 찾은 오버헤드"였고,
그 절반을 한 줄짜리 메모로 없앴다. 27B 판정이 0.660x → 0.780x 로 움직였다.

남은 8.25 ms 를 전부 없애면 53.78 → 45.5 ms 가 되고 사슬(43.34)의 **0.95x** 다.
거기에 온도 0.7 에서 관측된 수용 이득(+1%)이 얹힌다. 즉 **동률 근처까지는 갈 수
있고, 이기려면 수용을 더 벌어야 한다** — 그게 407 문서의 Weaver 가 하는 일이다.

🔴 이 장은 §42·§46 의 27B 수치를 **전부 대체한다** (0.660x → 0.780x).

# 48. 남은 6.15 ms — GPU 유휴다 (2026-09-02)

## 48-1. 이분법을 끝까지

메모(§47) 이후 남은 8.25 ms 를, 사슬 모양·마스크 생략에 조각을 더 빼며 좁혔다.

| 구성 | ms/스텝 | A 대비 | 뺀 값 |
|---|---|---|---|
| A DFlash2 사슬 | 43.34 | 기준 | |
| NOGDN | 49.48 | +6.15 | GDN 잔여 2.11 |
| + 마스크 조기이탈 | 49.41 | +6.07 | 0.07 |
| + NOROPE | 49.19 | +5.86 | 0.21 |
| + KV압축 없음 | 49.15 | +5.81 | 0.05 |
| + D2H 페이크 | 48.83 | +5.50 | **0.32** |

D2H 전송을 통째로 가짜로 만들어도 **0.32 ms** 밖에 안 준다. 고정(pinned) 버퍼로
바꿔도 마찬가지였다 (B -0.33, D **+0.30** — 효과 없음, 되돌렸다).

## 48-2. 프로파일러가 답을 준다

torch 프로파일러로 두 팔을 비교했다 (24토큰 1프롬프트, 약 7.5 스텝):

| | 총 CUDA | 총 CPU |
|---|---|---|
| 사슬 | 329.3 ms | 289.2 ms |
| 최소 DDTree | 331.9 ms | 319.6 ms |

**GPU 작업은 +2.6 ms 뿐이고 CPU 가 +30.4 ms 다.** 최대 항목은 `cudaMemcpyAsync`
+25.73 ms / 27회 = 회당 약 0.95 ms 인데, **전송 비용이 아니다** — 고정 버퍼가 안
먹힌 것이 그 증거다. 그건 **CPU 가 GPU 스트림을 기다리는 시간**이다.

벽시계와 맞춰 보면 확정된다:

| | CUDA | 벽시계 | GPU 바쁨 | 유휴 |
|---|---|---|---|---|
| 사슬 | 329.3 ms | 325 ms | **101%** | 0 |
| 최소 DDTree | 331.9 ms | 369 ms | **90%** | **4.90 ms/스텝** |

**측정 격차 5.81 ms 중 4.90 ms 가 GPU 유휴다.**

## 48-3. 원인은 구조적이다

사슬 스펙 디코딩은 검증을 **전부 GPU 에서** 끝낸다 — 기각 샘플러가 GPU 커널이고
CPU 는 개입하지 않는다. 그래서 GPU 가 101% 바쁘다(완전히 파이프라인된다).

DDTree 는 스텝마다 **CPU 로 내려와 트리를 걷는다** (`follow_tree` 는 파이썬
`child_maps` 순회다). 그 지점에서 CPU 가 GPU 를 기다리고, 기다리는 동안 다음
작업을 큐에 못 넣어 GPU 가 논다. 27B 는 스텝이 43 ms 라 그 버블이 크다.

8B 에서 D2H 를 0.056 ms 로 재고 "GPU 트리 구성은 값이 없다"고 한 것(§39-1)은
**8B 기준으로는 맞았다** — 거기선 기계값 자체가 2.41 ms 였다. 27B 처럼 스텝이
길고 큐가 깊으면 같은 왕복이 훨씬 비싸다.

## 48-4. 17.99 ms 의 최종 회계

| 항목 | ms/스텝 | 상태 |
|---|---|---|
| `gdn_info` 계층마다 재생성 | **9.77** | ✅ 고침 (§47) |
| GPU 유휴 (CPU 트리 워크 왕복) | **~4.90** | 구조적 |
| GDN 트리 경로 잔여 | 2.11 | 미해결 |
| D2H 전송 | 0.32 | 구조적 |
| RoPE | 0.21 | |
| KV 압축 | 0.05~0.56 | |
| 마스크 배열 생성 | 0.07 | ✅ 고침 |
...
**현재: 63.55 → 약 53.8 ms/스텝, 23.64s → 20.08s, 0.660x → 0.781x.**
출력은 시작 시점과 6/6 바이트 동일. (KV 압축 항목은 §49 에서 -0.083 ms 더 줄었다.)

## 48-5. 그래서 다음은

남은 것의 절반이 **CPU 트리 워크가 만드는 GPU 유휴**다. 이걸 없애려면
`follow_tree`(수용)와 트리 구성을 **GPU 로 옮겨야** 한다 — 그러면 스텝 안에
CPU 왕복이 사라진다. §39-1 에서 "D2H 가 0.056 ms 라 값 없다"고 접었던 그
작업인데, **27B 기준으로는 4.9 ms/스텝짜리다.**

그걸 다 없애면 약 49 ms → 사슬(43.34)의 0.88x. 나머지 격차는 GDN 잔여 2.11 과
검증 폭이다. 여전히 이기려면 수용을 더 벌어야 한다.

# 49. 동기화 지점을 개수로 세다 — 압축 검사 제거 (2026-09-02)

## 49-1. 스텝당 블로킹 D2H 는 4개가 아니라 3개였다

코드를 다시 세면:

| 위치 | 무엇 | 상태 |
|---|---|---|
| `tree.py` 드래프터 top-k | lp + ids | **이미 1회로 합쳐져 있었다** (`50ffc38f11`) |
| `runtime.py:accept` | 타깃 argmax → `follow_tree` | 남음 |
| `runtime.py:compact` | `(dst>src).sum()` 안전성 검사 | **이번에 제거** |

🔴 `tree.py` 의 병합을 "미구현" 이라고 읽은 것은 **stale worktree** (`wt-perf`,
브랜치 `ddtree-perf`, 812f6070c0) 를 본 탓이다. 정본은 `/home/user/vllm/vllm`
의 `ddtree-multireq` 다. 사본이 넷(mnt, mnt_head, mnt_perf, mnt_v2)에 워크트리가
둘이라 **어느 것이 도는지 먼저 확인할 것** — 라이브 마운트는 `mnt_v2/ddtree` 다.

## 49-2. 압축 안전성 검사는 왜 있었나

triton 커널이 세그먼트의 행을 증가 순서로 순차 처리하면서 `dst<=src` 를
**슬롯 공간에서** 전제했다. 트리가 KV 블록 경계를 넘으면 다음 블록 id 가 더
작을 수 있어(블록은 free pool 에서 나온다) 전제가 깨진다. 그래서 매 스텝
D2H 로 검사하고 걸리면 torch 경로로 빠졌다.

해법이 두 개였다:

  - **torch 로 통일** — 전제가 없으니 검사도 없다. §전(前) 실측에서 **순손해**
    였다. 커널이 계층마다 permute/index_select/index_copy_ 라 런치가 수십 배다.
  - **전제를 없앤다** — 커널이 행을 **전부 읽은 뒤 전부 쓴다**. gather 의미가
    그대로다. 프로그램끼리는 원래 안 겹친다(세그먼트=슬롯, fblk=바이트 구간,
    layer=캐시). 이쪽으로 갔다.

## 49-3. 반례가 진짜인지부터 확인했다

t5 에 슬롯 역전 2건을 넣고 **예전 커널로도 돌려봤다**:

| 경우 | 옛(순차) | 새(2단계) |
|---|---|---|
| 슬롯 역전 (블록 경계) | 🔴 오차 4.4375 | 오차 0 |
| 역전 + 정상 세그먼트 혼합 | 🔴 오차 3.4453 | 오차 0 |
| 나머지 7건 | 오차 0 | 오차 0 |

옛 커널이 실제로 틀린 값을 쓴다 — 반례가 무의미하지 않다. 즉 이건 성능 변경일
뿐 아니라 **전제 자체를 없앤 정확성 개선**이다. (`compact_unsafe` 는 실측에서
0건이었으므로 운영 중에 이 버그가 난 적은 없다.)
...
| 경우 | 옛(순차) | 새(2단계) | 차이 |
|---|---|---|---|
| 27B 배치1 수용4 | 160.6us | 162.8us | +2.2us |
| 27B 배치8 수용4 | 158.9us | 163.8us | +5.0us |
| 27B 배치64 수용4 | 196.6us | 197.6us | +1.0us |

커널 자체는 사실상 공짜다. 끝에서 끝까지 (27B AWQ + DFlash2, 하드, 예산 7,
β=-0.3, 팔을 번갈아 6쌍, 팔당 12표본):

| | 중앙값 | 표준편차 |
|---|---|---|
| 옛 (검사 있음) | 20.148s | 0.029 |
| 새 (검사 없음) | **20.084s** | 0.032 |

**-0.32%, 6/6 쌍에서 새가 빠름, 부호검정 p=0.031.** 출력 6/6 바이트 동일.

## 49-5. 🔴 "동기화 1회 = 0.95 ms" 는 틀렸다 — 철회

§48-3 에서 프로파일러의 `cudaMemcpyAsync` 합계를 호출 수로 나눠 **0.95 ms/회**
를 얻고, 그걸 모든 동기화 지점에 똑같이 적용했다. 실측하면:

  - 압축 검사의 `c_viol` = 0.0537s / 264회 = **0.20 ms/회**
  - 압축이 도는 스텝은 34% 뿐 (나머지는 항등 빠른 경로) → **0.068 ms/스텝**

끝에서 끝까지 -0.064s / 769스텝 = **-0.083 ms/스텝** 으로 정확히 맞는다.

**동기화 지점마다 값이 다르다.** 0.95 ms 는 평균이었고, 그 평균을 만든 것은
스칼라 하나를 읽는 이 검사가 아니라 **뒤에 무거운 CPU 작업이 붙어 있는** 두
지점이다 — 드래프터 top-k(뒤에 파이썬 트리 빌드) 와 argmax(뒤에 `follow_tree`).
버블의 크기는 전송량이 아니라 **읽은 뒤 CPU 가 붙잡고 있는 시간**이 정한다.
그래서 §48-5 의 결론(트리 워크를 GPU 로)은 그대로 유효하다 — 오히려 그 두
지점에만 값이 몰려 있다는 뜻이라 더 선명해졌다.

## 49-6. 지금

**53.91 → 약 53.8 ms/스텝, 0.778x → 0.781x.** 작다. 하지만 이건 순서 전제를
없앤 정확성 개선이기도 하고, 남은 유휴가 어디 있는지 **소거법으로** 확정해
준다 — 작은 스칼라 D2H 는 값이 없다. 값은 CPU 작업이 붙은 지점에만 있다.

## 49-7. 포인터 텐서 메모 — 구간은 줄고 끝에서 끝까지는 안 잡힌다

`compact_kv_triton` 이 캐시 `data_ptr` 리스트를 **호출마다** H2D 로 올리고
있었다. 포인터는 할당 뒤로 안 바뀐다.

| | |
|---|---|
| 포인터 텐서만 만들기 | 28.7us |
| 전체 (매번 새로) | 59.4us |
| 전체 (포인터 재사용) | **26.7us** (-32.7us, 호출의 55%) |

구간은 예측대로 줄었다 — `kv_compact` 0.3415s → 0.2950s (**-0.047s**,
-0.061 ms/스텝). 32.7us × 1320회 = 0.043s 와 맞는다.

**그런데 끝에서 끝까지는 안 잡힌다:** 6쌍 번갈아 **3/6, p=1.000, 중앙값 +0.01%**.

🔴 이 창의 잡음이 sd **0.5%** 로 효과(0.3%)보다 크다. 같은 섀시의 GPU1 이 운영
트래픽으로 92°C·100% 라 GPU0 클럭이 흔들렸다 — 앞 실험(§49-4)의 sd 0.15% 의
3배다. 이 조건에서 0.3% 를 가르려면 40쌍쯤 필요하다. **측정이 부족한 것이지
"효과 없음" 이 증명된 것이 아니다.**

다만 §48 과 들어맞는 해석이 있다: 27B DDTree 는 **GPU 가 90% 만 바쁘다.**
GPU 타임라인에서 깎은 일은 이미 있는 버블에 흡수된다. 일이 준 것은 확실하니
남기되(무료·정확), **끝에서 끝까지를 늘리려면 버블 자체를 없애야 한다.**

## 49-8. §49 가 정리한 것

  - 값은 **동기화 개수**가 아니라 **동기화 뒤에 붙은 CPU 작업**에 있다 (49-5).
  - GPU 타임라인 작업을 깎는 것도 **버블이 있는 동안은 안 보인다** (49-7).
  - 둘 다 같은 곳을 가리킨다 — 드래프터 top-k 와 argmax 뒤의 파이썬 트리
    작업. **거기 말고는 깎을 곳이 없다.**

# 50. 수용을 벌 자리가 어디인지 — 오라클 트리로 잰다 (2026-09-02)

질문: "DFlash2 사슬보다 DDTree 가 빨라질 수 있는 길과, 수용률을 벌 방법."
§48-5 는 "기계값을 다 없애도 0.88x 라 수용을 더 벌어야 한다" 로 끝났다. 그러면
**수용을 어디서 얼마나 벌 수 있는지**를 먼저 재야 한다. 추측이 아니라.

## 50-1. 방법 — 큰 트리 한 번이면 작은 모양 전부를 안다

노드 i 의 `sampled[i]` 는 '조상 문맥에서 타깃이 뽑은 토큰' 이라(트리 마스크의
정의), 그 노드를 포함하는 **어떤 부분 트리에서도 그대로 유효**하다. 그래서
척추 7 + 형제(예산 16, `VLLM_DDTREE_SPINE=1`)를 trace 와 함께 한 번 돌리면
사슬·형제 하나·형제 둘… 모든 부분 모양의 수용을 오프라인에서 계산할 수 있다.

trace 에 두 가지를 더 넣었다 (`ids_top`, `lp_top16` — trace 를 켠 스텝에만 붙는다):
**깊이별 드래프터 상위 16 후보 id 와 log-prob**. 이게 있어야 "트리에 안 들어간
형제" 까지 포함해 **선택 편향 없이** 형제 적중률을 잰다 (§34-2 의 강제 모양 없이).
분석은 `scratchpad/oracle_an.py`.

🔴 **운영 샘플링 조건도 같이 쟀다.** 운영 서버는 `--generation-config` 를 안
주므로 vLLM 기본값(auto)이 모델의 `generation_config.json` 을 요청 기본값으로
쓴다 — **T=1.0, top_k=20, top_p=0.95**. 온도를 안 주는 클라이언트는 전부 이
조건이다. §45 는 T=0.7 만 봤다. 하네스에 `DDT_STOPK`/`DDT_STOPP` 를 넣었다.

🔴 **T>0 비교가 공정한 이유.** 운영 사슬은 그리디 드래프트라(`draft_sample_method`
기본값 greedy) 기각 샘플러가 `NO_DRAFT_PROBS` 경로를 탄다 —
`rejection_sampler.py:816` 에서 `draft_prob = 1` 이므로 **p(x) 확률로 수용**한다.
DDTree 의 '노드마다 뽑고 따라가기'(§45-1)도 노드 단위로 정확히 p(x) 다. 즉 두 팔의
척추 수용 규칙이 같고, 차이는 형제뿐이다.

조건: 27B 운영 모델 + DFlash2, V2 러너, cudagraph, hard 표본 6 × 192 토큰, 배치 1.
운영 GPU1 은 유휴(54°C) 였다 — 규칙 7 의 잡음 조건이 좋은 창이다.

## 50-2. 끝에서 끝까지

| 조건 | 구성 | 토큰/스텝 | 스텝 | ms/스텝 | tok/s | 사슬 대비 |
|---|---|---|---|---|---|---|
| T=0 | **DFlash2 사슬 예산7** | **3.200** | 360 | 43.35 | **73.8** | 1.000x |
| T=0 | 척추7+형제1 (예산8, 9토큰) | 3.200 | 360 | 60.77 | 52.7 | 0.714x |
| T=0 | 척추7+형제2 (예산9, 10토큰) | **3.329** | **346** | 62.04 | 53.7 | 0.727x |
| T=0 | 〃 cudagraph 캡처크기 10 추가 | 3.291 | 350 | 62.07 | 53.0 | 0.718x |
| T=0 | 척추7+형제5 (예산12) | 3.273 | 352 | 61.98 | 52.8 | 0.715x |
| T=0 | 척추7+형제9 (예산16) | 3.310 | 348 | 63.23 | 52.4 | 0.709x |
| T=0 | 척추7+형제25 (예산32) | 3.282 | 351 | 72.53 | 45.2 | 0.613x |
| 운영 | **DFlash2 사슬 예산7 (그리디 드래프트)** | **2.859** | 403 | 43.58 | **65.6** | 1.000x |
| 운영 | 사슬 예산7, 확률적 드래프트+기각샘플링 | 2.931 | 393 | 43.79 | 66.9 | 1.020x |
| 운영 | DDTree 예산7 β=-0.3 (현행) | 2.433 | 471 | 53.68 | 45.3 | 0.691x |
| 운영 | 척추7+형제9 (예산16) | 2.730 | 422 | 62.20 | 43.9 | 0.669x |

(T=0 은 사슬과 같은 세션·같은 코드로 다시 잰 값이고 §42/§48 과 일치한다. 운영
조건은 시드 1234 이지만 팔마다 난수 소비가 달라 **텍스트가 서로 다르다** —
n=1 이라 ±5% 는 텍스트 잡음이다. 확률적 드래프트의 +2.5% 도 그 안이다.)

세 가지가 바로 보인다.

  1. **수용 이득은 형제 둘에서 끝난다.** 예산 9 가 346 스텝으로 최고이고
     12/16/32 는 348~352 로 같다(잡음 ±3). 형제를 9개, 25개 더 줘도 한 스텝도
     더 못 번다. 이득은 사슬 대비 **+3.5~4%** 다.
  2. **단가는 8→9 토큰에서 계단으로 뛴다** (53.8 → 60.8 ms) — 50-5.
  3. **운영 샘플링 조건에서는 현행 DDTree(예산 7)가 수용에서도 진다** (2.433
     대 2.859). 형제가 척추 자리를 뺏는 손해가 T=1 에서 더 크다.

## 50-3. 형제는 잎 하나 값이다 — 왜 예산 9 에서 포화하는가

trace 조건부 확률 (T=0, 예산 16, 340 스텝):

**첫 불일치 지점에서 타깃 토큰이 드래프터 상위 몇 위인가** (전체 313건):

| ≤1 | ≤2 | ≤3 | ≤5 | ≤15 | 밖 |
|---|---|---|---|---|---|
| 0.371 | 0.518 | 0.575 | 0.658 | 0.802 | 0.198 |

즉 척추가 틀리는 자리의 37% 는 **rank-1 형제가 정답**이고, 상위 3 이면 58% 다.
형제 자리는 분명히 있다. 그리고 드래프터는 그걸 **과소평가**한다:

| (깊이, rank) | 도달 | 실제 적중 | 말한 값 |
|---|---|---|---|
| (1, 1) | 84 | 0.333 | 0.234 |
| (2, 1) | 55 | **0.473** | 0.218 |
| (3, 1) | 40 | 0.325 | 0.238 |
...
반대로 척추는 **과대평가**한다 (d1 말한 값 0.776 → 실제 0.691, d2 0.795 → 0.668).
§16-3 의 4B DFlash 는 척추를 과소평가했는데 27B DFlash2 는 반대다 — 보정 상수는
체크포인트마다 다시 재야 한다.

**그런데 형제 뒤가 안 이어진다.** 형제가 맞은 뒤, 형제 문맥에서 타깃이 뽑은
토큰이 다음 깊이 주변분포(marginal)의 상위 몇 위인가 (112건):

| | rank0 | ≤2 | 상위16 밖 |
|---|---|---|---|
| 형제 뒤 | **0.223** | 0.33 | 0.196 |
| 척추 (비교) | 0.65~0.84 | | |

DFlash2 는 한 번의 forward 로 7개 위치의 **주변분포**를 낸다. 그 분포는 사실상
argmax 경로에 조건부다 — 위치 d+1 의 분포는 위치 d 가 rank-0 이라는 전제로
나온 것이라, 형제(rank-1) 뒤에서는 22% 만 맞는다. 예산 32 에서는 15% 다.
**그래서 형제는 잎 하나(+1 토큰) 값이고, 형제의 자식은 거의 쓰레기다.** 트리를
아무리 넓혀도 형제 둘 이상은 못 벌고, 깊이 방향으로는 전혀 못 번다.

이게 논문의 전제와 다른 지점이다. 논문은 주변분포 트리가 값을 한다고 했지만,
그건 형제 뒤 이어짐이 어느 정도 되는 드래프터에서다. DFlash2 는 척추가 정확한
대신 주변분포가 척추에 강하게 묶여 있다.

## 50-4. 🔴 규칙 9 — trace 반사실은 위치 편향이 있다

같은 trace 에서 "척추만 따라갔으면" 을 계산하면 1.985 수용/스텝인데, 사슬을
실제로 돌리면 2.200 (3.200−1) 이다. 트리 실행이 멈추는 위치는 "형제까지 다
틀린 자리" 라 그 다음 위치들이 체계적으로 더 어렵다 — 트리의 스텝 위치에서
잰 사슬은 실제 사슬보다 10% 낮다. 그래서 반사실 이득 +0.46/스텝(+15%) 이
끝에서 끝까지는 **+3.4%** 로 준다. 4배 차이다.

**한 trace 에서 계산한 부분 모양의 수용은 조건부 확률(적중률·순위)에만 쓴다.
토큰/스텝 비교는 반드시 끝에서 끝까지 스텝 수로 한다.** 50-2 표가 그것이다.

## 50-5. 단가 계단 — 8→9 토큰에서 +7 ms, 패딩이 아니다

| 스텝 토큰 수 | 8 | 9 | 10 | 13 | 17 | 33 |
|---|---|---|---|---|---|---|
| ms/스텝 | 53.8 | 60.8 | 62.0 | 62.0 | 63.2 | 72.5 |

9~17 이 평탄하고 8→9 만 계단이다. vLLM 의 cudagraph 캡처 크기가 `1,2,4,8,16,24…`
라 10 토큰이 16 으로 패딩되는 것을 먼저 의심했는데, **캡처 크기에 10 을 넣어도
62.07 로 동일**하다 (`DDT_CGSIZES`, 하네스 손잡이). 패딩이 아니다.

**프로파일이 답을 준다** (예산 7 대 9, 24토큰 1프롬프트 ≈ 7.6 스텝, `DDT_PROF`):

| | 예산 7 (T=8) | 예산 9 (T=10) |
|---|---|---|
| 총 CUDA | 332.3 ms | 358.1 (+25.8 = **+3.4 ms/스텝**) |
| 총 CPU | 333.0 ms | 458.8 (+125.9 = **+16.5 ms/스텝**) |
| GDN 재귀 커널 | `fused_gdn_decode_post_conv_mtp` (CUDA) 11.07 ms / 240회 | `fused_sigmoid_gating_delta_rule_update_kernel` (Triton) 14.86 / 240회 |
| `qwen_gdn_attention_core_fused_norm_packed` | 1.73 ms | **17.21** |
| `aten::copy_` / elementwise / DtoD / cat 호출 수 | 1760 / 150 / 91 / 21 | **3919 / 1590 / 571 / 286** |

**T ≤ 8 에서는 GDN 계층당 융합 CUDA 커널 하나**가 돌고, **T > 8 에서는 Triton
커널 + 패킹/정규화/복사 열 개 남짓의 비융합 경로**로 떨어진다 — 48 계층이라
스텝당 런치가 약 500개 늘어 **런치 바운드**가 된다 (CPU +16.5 ms 중 대부분이
GPU 와 겹치고 벽시계에는 +7 ms 로 남는다). 스톡 MTP 커널의 `state_indices
[N, S] S ≤ 8` 상한(§36-2 의 pad 주석)을 우리 CUDA 트리 커널이 그대로 물려받은
것이다. Marlin GEMM 은 템플릿만 바뀌고 +0.36 ms/스텝, 어텐션 +0.1 이다.

즉 **계단은 우리 커널의 용량 문제**다. CUDA 트리 커널을 T ≤ 16 으로 넓히면
(§46-1 대로 T 에 선형이므로 T=10 에서 계층당 약 +14 µs) 예산 9 의 단가는
약 **55 ms** 로 내려온다 — 지금의 62 가 아니라. 이게 좁은 트리의 진짜 폭 값이다.
사슬은 드래프터 지평이 8 이라 이 경로를 절대 안 탄다.

## 50-6. 운영 샘플링 조건에서

  - 사슬 2.859 → DDTree-7 2.433 (−15%). 척추 규칙이 같으니(50-1) 차이는 모양이다:
    예산 7 안에서 형제가 척추 자리를 뺏는데, T=1 에서는 척추가 약해져
    (d1 적중 0.529 대 T=0 0.691) 깊은 척추 자리의 값이 더 커진다.
  - 형제 적중은 T=0 과 비슷하다 (불일치에서 ≤3: 0.485, (2,1): 0.380).
    이어짐도 같은 구조다.
  - **트리 기각 샘플링(SpecInfer 류)은 그리디 후보에서는 무연산이다.** 후보 x₁,x₂
    를 순서대로 기각 샘플링하면 P(수용) = p₁ + (1−p₁)·p₂/(1−p₁) = p₁ + p₂ — '뽑고
    따라가기' 와 정확히 같다. 차이가 나려면 드래프터의 q 를 써야 하는데(확률적
    드래프트), 그게 사슬에서 +2.5%(n=1, 잡음 안) 다. 트리에서 더 크게 날 이유가
    없다. §1 의 "T>0 트리 기각 샘플링이 필수" 는 **철회** — 현행 규칙이 분포를
    보존하고(§45-1), 더 센 규칙도 없다.

## 50-7. 지렛대와 상한

배치 1, T=0, 사슬 73.8 tok/s 기준. 이기려면 tok/스텝 ÷ ms/스텝 이 이걸 넘어야 한다.

| 지렛대 | 실측 | 남는 상한 |
|---|---|---|
| 좁은 상위집합 트리 (척추7+형제2) | **+3.5~4% 토큰/스텝**, 형제 2 에서 포화 | 폭 값이 계단(50-5)에 달림 |
| 기계값: GPU 트리워크(§48 4.9) + GDN 잔여(2.1) | 미구현 | 예산 7 에서 46.8 ms → **0.88x** |
| 위 둘 + 커널 T≤16 확장 (예산 9 ≈ 55 ms → 기계값 빼면 48 ms) | — | 3.33 / 48 ms → **0.94x** |
| 조건부 재드래프트 (형제 뒤 22%→~65%) | trace +0.5/스텝 → e2e 약 +4% (50-4 의 4배 할인) | 드래프터 2회째 forward ≈ 8~10 ms (5층·5120 + 2.5 GB lm_head, 메모리 바운드) → **순손실** |
| 트리 기각 샘플링 | 그리디 후보에서 무연산 (50-6) | 0 |
| 넓은 트리 (12/16/32) | 포화 | 0 |
| 온도 (§45 + 50-2) | 트리 비율 오히려 하락 (0.78x → 0.69x) | 0 |
| 게이팅 (확신 스텝은 사슬) | 사슬 스텝이 DDTree 경로를 타면 +10 ms 그대로. 스톡 경로로 넘겨야 의미 | 이득 +2~3% 대 비용 +7% |
| 드래프터 재학습 (형제 보정, 조건부 분포, 도메인) | 미측정 | **유일하게 상한이 큰 축** — vLLM 밖 |

## 50-8. 판정

  - **트리 구성·검증 쪽에 남은 수용은 +3.5~4% 뿐이고 그건 형제 둘로 끝난다.**
    주변분포가 척추 조건부라 형제 뒤가 안 이어지기 때문이다 (22%).
  - 기계값을 전부 없애고 커널을 넓히고 좁은 트리를 써도 **0.94x** 다. 조건부 재드래프트는
    드래프터 forward 를 한 번 더 먹어 순손실이다.
  - 운영 샘플링 조건(T=1.0, top_k 20)에서는 현행 DDTree 가 수용에서도 진다.
  - **DFlash2 를 넘는 길은 드래프터 쪽뿐이다** — 형제 뒤 분포를 내놓는 드래프터
    (조건부 분포, 2단계 디노이징), 또는 형제를 덜 과소평가하도록 재학습.
    vLLM 통합에서 할 수 있는 일은 끝났다.
  - 수용 자체를 벌고 싶다면(트리와 무관하게): 운영 사슬의 확률적 드래프트는
    +2.5% 로 잡음 안이라 n=10 A/B 가 필요하고, 그 외에는 드래프터 재학습이다.
