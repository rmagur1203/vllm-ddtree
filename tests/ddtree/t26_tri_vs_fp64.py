...
# --- Triton 경로도 같은 입력으로 돌려 fp64 와 대조한다 ---
from vllm.third_party.flash_linear_attention.ops.fused_sigmoid_gating import (
    fused_sigmoid_gating_delta_rule_update)
q = mixed[:, :H*DK].view(1, T, H, DK).contiguous()
k = mixed[:, H*DK:2*H*DK].view(1, T, H, DK).contiguous()
v = mixed[:, 2*H*DK:].view(1, T, HV, DV).contiguous()
st_tri = state.clone()
tri_pre, _ = fused_sigmoid_gating_delta_rule_update(
    A_log=A_log, a=a.unsqueeze(0), b=b.unsqueeze(0), dt_bias=dt_bias, q=q, k=k, v=v,
    initial_state=st_tri, inplace_final_state=True, cu_seqlens=cu,
    ssm_state_indices=si, num_accepted_tokens=nacc,
    tree_parent_indices=par, use_qk_l2norm_in_kernel=True)
tri_pre = tri_pre.reshape(T, HV, DV)
# 커널과 같은 후처리(그룹 RMSNorm 후 게이트)를 fp32 로 적용
tp = tri_pre.float()
rstd = torch.rsqrt(tp.pow(2).mean(dim=-1, keepdim=True) + EPS)
tri_out = tp * rstd * nw.float() * torch.nn.functional.silu(gate.float())
print(f"  Triton 출력 vs fp64 참조  상대오차 {rel(tri_out, ref_out):.6f}")
_t = si[0].long().unique()
print(f"  Triton 상태 vs fp64 참조  상대오차 {rel(st_tri[_t], ref_st[_t]):.6f}")
print(f"  CUDA vs Triton (출력)     상대오차 {rel(cuda_out, tri_out):.6f}")

# 🔴 공정 비교: Triton pre-norm 을 bf16 으로 반올림한 뒤 같은 정규화를 건다.
#    우리 커널은 shared_out 을 bf16 으로 두므로 이 조건이 동일하다.
tp_b = tri_pre.to(torch.bfloat16).float()
rstd_b = torch.rsqrt(tp_b.pow(2).mean(dim=-1, keepdim=True) + EPS)
tri_bf = tp_b * rstd_b * nw.float() * torch.nn.functional.silu(gate.float())
print(f"  Triton(bf16 반올림) vs fp64  상대오차 {rel(tri_bf, ref_out):.6f}")
# 🔴 최종 저장 조건도 맞춘다. 우리 커널은 out 을 bf16 으로 쓰는데 Triton 경로는
#    fp32 로 남아 있었다 — 이 상태로 비교하면 커널이 부당하게 나빠 보인다.
print(f"  Triton→bf16 저장 vs fp64     상대오차 {rel(tri_out.to(torch.bfloat16), ref_out):.6f}")
print(f"  참고: fp64 를 bf16 으로만 저장 {rel(ref_out.to(torch.bfloat16), ref_out):.6f}  ← 저장 자체의 하한")
