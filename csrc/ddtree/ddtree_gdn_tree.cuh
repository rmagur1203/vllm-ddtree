...
// 커널 본체 — 텐서 타입에 의존하지 않는다. at::Tensor 래퍼(JIT)와
// torch::stable::Tensor 래퍼(빌드 편입)가 함께 포함한다.
#pragma once
#include <cuda_bf16.h>
...
// vhpk(= value head / key head) 디스패치. 두 래퍼가 함께 쓴다.
// 오류 보고는 ABI 마다 다르므로 래퍼가 DDT_FAIL_VHPK 를 정의한다.
