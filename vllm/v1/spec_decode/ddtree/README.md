...
### V1 과 V2 — 무엇을 공유하고 무엇이 다른가

`ddtree/` 패키지는 **V1·V2 가 공유한다.** 개발용 V2 마운트
(`ddtree-dev/mnt_v2/ddtree/`)는 이 디렉터리의 사본이고 2026-09-02 에 맞췄다.
바꿀 때는 **양쪽 다** 갱신할 것 — 갈라지면 측정이 어느 쪽 코드에서 나왔는지
알 수 없게 된다 (실제로 갈라져 있었다, docs §49-1).

동일성 근거 (`scratchpad/equiv_gdn.py`, CPU): V2 가 실제로 도는 조건
(모든 요청이 트리를 갖고 크기가 같다)에서 예전 V2 경로(`tree_cols_tensor`)와
현재 경로(`cols_from_parents`)가 **400 케이스 전부 비트 단위로 같은**
`tree_cols`/`parents` 를 낸다. 현재 경로는 그 위에 요청별 길이가 다른 배치와
트리 없는 요청이 섞인 배치까지 처리한다 (예전 경로는 None 으로 물러서는데,
물러선 자리가 8토큰 상한이 있는 스톡 커널이라 예산>8 이면 죽는다).

**남아 있는 차이는 소비자 쪽이다.** 런타임은 요청별 드래프트 길이를
`draft_lens` 로 알려주는데 **V1 러너만 그걸 읽는다** (`gpu_model_runner.py`).
V2 패치는 안 읽는다. 기본 설정에서는 `pad_to_budget` 이 모든 트리를 예산까지
채우므로 길이가 균일해 차이가 없고, 드물게 짧은 트리가 나오면 V2 에서는 그
스텝의 스펙 디코딩이 그냥 안 붙는다(무손실, 손해는 속도뿐). 🔴 다만
`VLLM_DDTREE_TRUECHAIN` + `VLLM_DDTREE_TAU` 를 **다중 요청과 함께** 켜면 V2 에서
폭이 어긋난다 — 아래 §업스트림 6번.
...
🔴 반드시 **컨테이너에서 뽑은 원본**에 적용할 것. 리포본은 리비전이 달라
   앵커가 안 맞는다. 같은 이유로 `flashinfer.py` 와 `qwen_gdn_linear_attn.py` 는
   리포본을 V2 마운트에 복사하면 안 된다 — 컨테이너 리비전에 우리 편집을 얹은
   별도 사본이다. `flashinfer.py` 의 편집은 양쪽이 같고,
   `qwen_gdn_linear_attn.py` 는 리포본에만 `_ddtree_active()` 가드(§21, DDTree 를
   **안 쓰는** 사용자에게 물리던 계층당 D2H 2회를 없앤다)와 `validate_tree_inputs`
   배선이 있다. 둘 다 DDTree 를 켠 V2 실행에는 영향이 없다.
...
6. **다중 요청에서 짧은 드래프트 미지원.** V1 은 `draft_lens` 로 요청별 길이를
   넘겨 처리하지만 **V2 패치는 그걸 안 읽는다.** 확신 시 드래프트를 짧게 내는 경로는
